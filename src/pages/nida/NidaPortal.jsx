...
import { StatCard, StatusBadge, Badge, Table, SectionHeader, Modal, MiniStat, Card } from '../../components/ui'
import { getStats, mockStudents, mockInstructors, mockComplaints, mockAdmissions, mockAttendance, mockAssignments, mockQuizzes, mockTasks, mockCourses, addAdmission, updateAdmission, updateTask } from '../../utils/mockData'
import { useAuth } from '../../context/AuthContext'
import { UserPlus, GraduationCap, Users, ClipboardList, AlertCircle, CheckSquare, BarChart3, Plus } from 'lucide-react'
  ...
  return (
    <div className="space-y-5 animate-fadeIn">
      <div className="rounded-2xl bg-gradient-to-r from-pink-500 to-rose-700 p-5 text-white">
        ...
        <h2 className="font-bold text-2xl mt-0.5">Nida 🌟</h2>
...
function NidaAdmissions() {
  ...
  const [admissions, setAdms] = useState(mockAdmissions)
  ...
  const add = ()=>{
    ...
    addAdmission({...form,appliedDate:new Date().toISOString().split('T')[0]})
    setAdms([...mockAdmissions]); setModal(false)
    setForm({name:'',email:'',phone:'',program:'Maritime Navigation',notes:''})
  }
  const updateStatus = (id,status,interviewDate='')=>{
    updateAdmission(id,{status,interviewDate})
    if(status==='accepted') pushNotif('student','Your admission has been accepted! Welcome to NMA.', 'admission')
    setAdms([...mockAdmissions]); setDetModal(null)
  }
  return (
    <div className="space-y-4 animate-fadeIn">
      <div className="flex justify-between items-center">
        <div className="flex gap-2"><MiniStat label="Pending" value={admissions.filter(a=>a.status==='pending').length} color="amber"/><MiniStat label="Interview" value={admissions.filter(a=>a.status==='interview').length} color="purple"/><MiniStat label="Accepted" value={admissions.filter(a=>a.status==='accepted').length} color="green"/></div>
...
function NidaStudents() { return <Card className="animate-fadeIn"><SectionHeader title="All Students"/><Table columns={[{key:'name',label:'Name',render:v=><span className="font-medium">{v}</span>},{key:'email',label:'Email'},{key:'phone',label:'Phone'},{key:'attendanceRate',label:'Att.',render:v=><span className={v>=80?'text-green-600':v>=60?'text-amber-500':'text-red-500'}>{v}%</span>},{key:'gpa',label:'GPA'},{key:'status',label:'Status',render:v=><StatusBadge status={v}/>},{key:'admissionDate',label:'Admitted'}]} data={mockStudents}/></Card> }
function NidaInstructors() { return <Card className="animate-fadeIn"><SectionHeader title="All Instructors"/><Table columns={[{key:'name',label:'Name',render:v=><span className="font-medium">{v}</span>},{key:'sub',label:'Role'},{key:'email',label:'Email'},{key:'students',label:'Students',render:v=><Badge color="blue">{v}</Badge>},{key:'status',label:'Status',render:v=><StatusBadge status={v}/>}]} data={mockInstructors}/></Card> }
function NidaAttendance() {
  const [cid,setCid]=useState('')
  const recs=mockAttendance.filter(a=>cid===''||a.courseId===parseInt(cid))
  return <Card className="animate-fadeIn"><div className="flex gap-3 mb-4"><select className="input text-xs py-1.5 w-60" value={cid} onChange={e=>setCid(e.target.value)}><option value="">All Courses</option>{mockCourses.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div><Table columns={[{key:'studentName',label:'Student'},{key:'courseId',label:'Course',render:v=>mockCourses.find(c=>c.id===v)?.name?.split(' ')[0]||`#${v}`},{key:'date',label:'Date'},{key:'status',label:'Status',render:v=><StatusBadge status={v}/>}]} data={recs}/></Card>
}
function NidaAssignments() { return <Card className="animate-fadeIn"><SectionHeader title="All Assignments"/>{mockAssignments.map(a=><div key={a.id} className="mb-3 p-3 border border-gray-100 rounded-xl"><div className="flex justify-between mb-1"><p className="font-semibold text-sm">{a.title}</p><Badge color="amber">{a.submissions.length} submissions</Badge></div><p className="text-xs text-gray-400">{mockCourses.find(c=>c.id===a.courseId)?.name} · Due: {a.dueDate}</p></div>)}</Card> }
function NidaQuizzes() { return <Card className="animate-fadeIn"><SectionHeader title="All Quizzes"/>{mockQuizzes.map(q=><div key={q.id} className="mb-3 p-3 border border-gray-100 rounded-xl"><div className="flex justify-between mb-1"><p className="font-semibold text-sm">{q.title}</p><Badge color="purple">{q.submissions.length} submitted</Badge></div><p className="text-xs text-gray-400">{mockCourses.find(c=>c.id===q.courseId)?.name} · {q.questions.length} questions</p></div>)}</Card> }
function NidaComplaints() { return <Card className="animate-fadeIn"><SectionHeader title="All Complaints"/><Table columns={[{key:'from',label:'From'},{key:'fromRole',label:'Type',render:v=><Badge color="gray">{v}</Badge>},{key:'subject',label:'Subject'},{key:'priority',label:'Priority',render:v=><Badge color={v==='high'?'red':v==='medium'?'amber':'green'}>{v}</Badge>},{key:'status',label:'Status',render:v=><StatusBadge status={v}/>},{key:'date',label:'Date'}]} data={mockComplaints}/></Card> }
function NidaTasks() {
  const [tasks,setTasks]=useState(mockTasks)
  const update=(id,status)=>{updateTask(id,status);setTasks([...mockTasks])}
  const myTasks=tasks.filter(t=>t.assignedToRole==='nida')
  return <div className="space-y-3 animate-fadeIn">{myTasks.length===0&&<Card className="text-center py-8 text-gray-400 text-sm">No tasks assigned</Card>}{myTasks.map(t=><Card key={t.id}><div className="flex justify-between mb-2"><p className="font-semibold">{t.title}</p><div className="flex gap-2"><Badge color={t.priority==='high'?'red':t.priority==='medium'?'amber':'green'}>{t.priority}</Badge><StatusBadge status={t.status}/></div></div><p className="text-xs text-gray-400 mb-2">Due: {t.dueDate}</p>{t.status!=='completed'&&<div className="flex gap-2">{t.status==='pending'&&<button onClick={()=>update(t.id,'in-progress')} className="btn-secondary text-xs py-1.5">Start</button>}{t.status==='in-progress'&&<button onClick={()=>update(t.id,'completed')} className="btn-primary text-xs py-1.5">Mark Complete</button>}</div>}</Card>)}</div>
}
function NidaRecords() {
  const stats=getStats()
  return <div className="space-y-4 animate-fadeIn"><div className="grid grid-cols-2 lg:grid-cols-4 gap-3"><MiniStat label="Total Students" value={stats.totalStudents} color="blue"/><MiniStat label="At-Risk" value={stats.atRisk} color="red"/><MiniStat label="Instructors" value={stats.totalInstructors} color="indigo"/><MiniStat label="Avg GPA" value={stats.avgGpa} color="green"/></div><Card><SectionHeader title="Student Records"/><Table columns={[{key:'name',label:'Name',render:v=><span className="font-medium">{v}</span>},{key:'email',label:'Email'},{key:'attendanceRate',label:'Att.',render:v=><span className={v>=80?'text-green-600':v>=60?'text-amber-500':'text-red-500'}>{v}%</span>},{key:'gpa',label:'GPA'},{key:'courses',label:'Courses',render:v=><Badge color="blue">{v?.length||0}</Badge>},{key:'status',label:'Status',render:v=><StatusBadge status={v}/>}]} data={mockStudents}/></Card></div>
}
